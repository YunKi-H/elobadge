    ...
    <article className="mx-auto max-w-3xl text-slate-300">
      ...
      <PolicySection title="2. 처리하는 개인정보">
        <PolicyTable
          ...
          rows={[
            ...
            [
              "서비스 이용 기록",
              "요청 경로, 응답 상태·시간, 오류 유형, 인증 사용자 Firebase UID, 스트리머 방송 플랫폼 ID, 채팅 길이·배지 수·역할 등 비원문 진단 정보",
              "서비스 이용 과정에서 자동 생성"
            ],
            [
              "브라우저 저장 정보",
        ...
        <p className="mt-4">
          ...
          토큰은 계정 확인 직후 폐기하며 저장하지 않습니다. 실시간 채팅
          이벤트의 발신자 플랫폼 ID는 연결된 EloBadge 계정과 레이팅을
          매칭하고 오버레이에 전달하기 위해 서버 메모리에서 최대 10분간
          캐시될 수 있습니다. 일반 채팅 참여자의 발신자 ID·닉네임·메시지
          원문은 운영 로그나 서버 데이터베이스에 저장하지 않습니다.
        </p>
      </PolicySection>
      ...
      <PolicySection title="3. 보유 및 이용 기간">
        <PolicyTable
          ...
          rows={[
            ...
            ["실시간 채팅 데이터", "발신자 ID는 레이팅 매칭 캐시로 서버 메모리에 최대 10분, 닉네임·메시지 원문은 오버레이 표시 시간 동안 메모리에서 처리하며 서버 데이터베이스에 저장하지 않음. 단, 이용자가 EloBadge에 직접 연결한 방송 플랫폼 계정 ID는 계정 연결 정보의 보유 기간을 따름"],
      ...
      <PolicySection title="10. 자동 수집 장치와 외부 자원">
        <p>
          ...
          또는 치지직·Twitch 배지 및 이모지 이미지를 요청할 때 해당 제공업체에
          IP 주소와 HTTP 헤더가 전달될 수 있습니다. 브라우저 저장 정보는 브라우저
          설정에서 삭제할 수 있으나 로그인 유지나 화면 표시에 영향을 줄 수 있습니다.
        </p>
      </PolicySection>
