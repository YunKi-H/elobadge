...
import { migrateChessBadgesToUsers } from "../firebase/chess-badge-migration.js";
...
const result = await migrateChessBadgesToUsers(execute);
...
console.log(
  `${result.candidates} candidate(s), ${result.unchanged} unchanged, ` +
    `${result.invalid} invalid, ${result.conflicts} conflict(s).`
);

if (execute) {
  console.log(`Updated ${result.migrated} user badge document(s).`);
} else {
  console.log("No document was changed.");
  console.log(
    "To execute: pnpm chess-badges:migrate --execute " +
